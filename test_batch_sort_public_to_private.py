...
class TestIOMetadata(unittest.TestCase):
    ...
    def setUp(self):
        self.test_folder = os.path.join(PUBLIC_ROOT, "1978", "tests", "test_01")
    ...
    def test_batch_sort_directory(self):
        ...
        private_test_folder = os.path.join(PRIVATE_ROOT, "1978", "tests", "test_01")
        logging.info(f"Verifying files moved to private root: {private_test_folder}")
        self.assertTrue(os.path.exists(private_test_folder), f"Files were not moved to private root: {private_test_folder}")
        for i in range(len(self.random_images)):
            self.assertTrue(any(f.startswith(f"test_image_{i+1}") for f in os.listdir(private_test_folder)), 
                            f"test_image_{i+1} was not moved to private root")
        
        # 2. Verify files deleted from public root
        logging.info(f"Verifying files deleted from public root: {self.test_folder}")
        self.assertFalse(os.path.exists(self.test_folder), f"Files were not deleted from public root: {self.test_folder}")
        
        # 3. Verify metadata in private_1978.json
        metadata_file = os.path.join(METADATA_FOLDER, "private", "private_1978.json")
        ...
        for i in range(len(self.random_images)):
            test_image_path = f"1978/tests/test_01/test_image_{i+1}"
            ...
            self.assertEqual(file_metadata["status"], "private", f"File status is not set to private for {test_image_path}")
            self.assertTrue(file_metadata["reviewed"], f"File is not marked as reviewed for {test_image_path}")
        
        # 4. Verify metadata not in public_1978.json
        public_metadata_file = os.path.join(METADATA_FOLDER, "public", "public_1978.json")
        if os.path.exists(public_metadata_file):
            ...
            for i in range(len(self.random_images)):
                ...
                self.assertFalse(any(key.startswith(test_image_path) for key in public_metadata.keys()), 
                                 f"Metadata found in public json file for {test_image_path}")
